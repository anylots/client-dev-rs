...
const PRIVATE_KEY: &str = "ebd24eb9345e93582be65937c87932f8b7e97c80c4d51e83b1e2bdc33c4acc42";
#[tokio::main]
...
async fn deploy() -> Result<(), Box<dyn Error>> {
    ...
    abigen!(TestZkEVM, "./resource/abi/TestZkEVM.json");
    let a: u64 = 10;
    // let testZkEVM = TestZkEVM::deploy(signer, a.pow(18)) .nonce(1)
    //     .unwrap()
    //     .send()
    //     .await;

    let tx = TestZkEVM::deploy(signer, a.pow(18))?.legacy();
    let testZkEVM = tx.send().await;
